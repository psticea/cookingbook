 ...
 * Price per serving is shown instead of difficulty
...
export const RecipeCard: React.FC<RecipeCardProps> = ({ recipe }) => {
  ...
  const recipeCost = calculateRecipeCost(recipe.ingredients, recipe.servings, language);
    ...
    >
      ...
      <div className="p-2 sm:p-3 flex items-center justify-between text-gray-600 dark:text-gray-400">
        ...
        <div className="flex items-center gap-1" title={`Price per serving`}>
          ...
          <span className="text-base font-medium">{formatPrice(recipeCost.pricePerServing, true)}</span>
        </div>
      </div>
    </Link>
  );
