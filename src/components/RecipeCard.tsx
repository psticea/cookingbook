...
export const RecipeCard: React.FC<RecipeCardProps> = ({ recipe }) => {
  ...
  // (generated by `npm run generate-thumbnails`).
  const baseImage = `${import.meta.env.BASE_URL}images/recipes/${recipe.category}/${recipe.id}`;
  const thumbUrl = `${baseImage}.thumb.webp`;
  const fullUrl = `${baseImage}.jpg`;
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-square w-full overflow-hidden bg-card-2-light dark:bg-card-2-dark">
        <img
          src={imageError ? defaultImageUrl : thumbUrl}
          alt={recipe.title[language]}
          className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
          loading="lazy"
          decoding="async"
          onError={(e) => {
            // If the .thumb.webp is missing, fall back to the full JPG before
            // showing the default placeholder.
            const img = e.currentTarget as HTMLImageElement;
            if (img.src.endsWith('.thumb.webp')) {
              img.src = fullUrl;
            } else {
              setImageError(true);
            }
