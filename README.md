...
- **User Preferences**: Normal/large text size, light/dark theme and language, set from the side menu; all settings persist in localStorage
...
- **Ingredient Cost Breakdown**: Expand the receipt below the checklist for costs at the selected serving count, a total, and a per-serving estimate. Unsupported units, missing prices, and invalid rates are marked unavailable rather than assigned a placeholder price. Incomplete estimates show a known subtotal and excluded ingredient count. Homepage recipe cards show the known cost per serving under the photo (e.g. `2.04 lei`), prefixed with `≈` for partial estimates whose details stay in the tooltip and accessible label; unavailable costs show `— lei`. Price sorting puts complete estimates first. Amounts represent ingredients used, not full shopping packages. Calculations use the English source unit in both display languages and retain the site's approximate spoon conversions; ingredient-specific density conversions are not inferred.
- **Personal Notes**: Add and save custom notes to any recipe for your own modifications
- **Responsive Design**: Mobile first: 2 columns of recipe photos on phones, 3 on tablets, 4 on desktop
...
- **Build Tool**: Vite 5
- **Styling**: Tailwind CSS driven by the "Light Table" design system — see [DESIGN.md](DESIGN.md) (tokens in `src/design/tokens.ts`, theme variables in `src/index.css`)
...
The site uses **The Light Table**: photographs carry the colour, the interface is a neutral gallery wall (light) or darkroom (dark), and one variable typeface (Archivo) sets the hierarchy. [DESIGN.md](DESIGN.md) is the normative reference for colours, type, spacing, components and rules; follow it for any new UI.

## Homepage Design Proposals

The Light Table was chosen from the [four-proposal comparison](mocks/homepage-four-proposals/index.html) (Market Grid, The Evening Kitchen, Enamel Pantry, The Light Table). An earlier [comparison gallery](mocks/homepage-proposals/index.html) preserves three older mockups: **Counterspace**, **The Kitchen Index**, and **Open Index**.
